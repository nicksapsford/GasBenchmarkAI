"""
watchdog_cryptobenchmark.py -- Galahad for GasBenchmark (Benchmark Desk).
Launches and supervises main_gasbenchmark.py: restarts it on unexpected exit,
...
TARGET = BASE_DIR / "main_gasbenchmark.py"

logging.basicConfig(level=logging.INFO,
                    format="%(asctime)s  %(levelname)-7s [Galahad] %(message)s",
                    datefmt="%Y-%m-%d %H:%M:%S")
logging.Formatter.converter = time.gmtime
log = logging.getLogger("GasBenchmark.Watchdog")
