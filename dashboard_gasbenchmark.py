"""
dashboard_gasbenchmark.py -- GasBenchmark A.I. dashboard (Benchmark Desk)
Port 5026. Single-instrument (Natural Gas). Prominent WITH/AGAINST switch (live
...
app = Flask(__name__)
_state = {"system": "GasBenchmark", "version": APP_VERSION}


HTML = """<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>GasBenchmark A.I.</title>
<style>
:root{--bg:#0d0d0f;--bg2:#16161a;--bd:#2a2a30;--tx:#e6edf3;--mut:#8b949e;
--gas:#228B22;--green:#3fb950;--red:#f85149;--amber:#d29922;}
...
header{background:var(--bg2);border-bottom:2px solid var(--gas);padding:10px 18px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;}
.brand{font-size:18px;font-weight:800;color:var(--gas);letter-spacing:1px;}
.brand small{color:var(--mut);font-size:11px;font-weight:400;letter-spacing:0;margin-left:8px;}
.clock{font-family:monospace;color:var(--gas);font-weight:700;}
...
.price{font-size:30px;font-weight:800;color:var(--gas);}
...
.lanc-clear{color:var(--green);} .lanc-block{color:var(--amber);} .lanc-trade{color:var(--gas);}
.note{color:var(--mut);font-size:10px;margin-top:14px;text-align:center;line-height:1.5;}
.nav{display:flex;align-items:center;gap:10px;}
.navbtn{font-size:12px;font-weight:700;color:var(--gas);background:rgba(255,255,255,0.06);border:1px solid var(--gas);padding:5px 12px;border-radius:6px;cursor:pointer;text-decoration:none;}
...
<header>
  <div class="brand">GAS<span style="color:#fff">BENCHMARK</span> A.I.
    <small>__VER__ &middot; port 5026 &middot; Natural Gas &middot; Lancelot + 3-TF SSL + switch</small></div>
  <div class="nav">
...
<div class="wrap">
  ...
  <div id="page2" style="display:none;">
    ...
    <div class="card">
      <div style="font-size:16px;font-weight:800;color:var(--gas);margin-bottom:12px;">Trade History &mdash; P&amp;L</div>
...
function poll(){
  fetch('/api/state').then(function(r){return r.json();}).then(function(d){
    var h='';
    h+='<div class="price">$'+(d.price!=null?Number(d.price).toFixed(3):'--')+'</div>';
    if(d.in_trade && d.position){
      ...
      h+=row('Entry','$'+Number(p.entry).toFixed(3));
      h+=row('Stop / Target','$'+Number(p.stop).toFixed(3)+' / $'+Number(p.target).toFixed(3));
...
TRADES_CSV = BASE_DIR / "logs" / "gas_trades.csv"
...
def api_health():
    return jsonify({"status": "ok", "system": "GasBenchmark",
                    "time": datetime.now(timezone.utc).isoformat()})


if __name__ == "__main__":
    print("GasBenchmark dashboard -> http://localhost:%d" % PORT)
